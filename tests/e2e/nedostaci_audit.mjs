...
await goto('/hr/vjestaci')
const vjSelects = await countSelects()
const vjCountyPh = await findPlaceholder('županij') || await findPlaceholder('county')
const vjCityPh = await findPlaceholder('grad') || await findPlaceholder('city')
const vjExpertPh = await findPlaceholder('područ') || await findPlaceholder('stručn') || await findPlaceholder('grana') || await findPlaceholder('expert')
...
await goto('/hr/tumaci')
const tuSelects = await countSelects()
const tuCountyPh = await findPlaceholder('županij') || await findPlaceholder('county')
const tuCityPh = await findPlaceholder('grad') || await findPlaceholder('city')
const tuLangPh = await findPlaceholder('jezik') || await findPlaceholder('lang')
...
// #5 Sudovi departments + hours
await goto('/hr/sudovi')
const firstCourt = page.locator('a[href*="/sudovi/"]').first()
if (await firstCourt.count() > 0) {
  await firstCourt.click().catch(() => {})
  ...
  const courtCity = await countText('/grad:/i') + await countText('/grad /i')
  ...
  log('NED#5 sudovi list', 'FAIL', 'no court detail links')
}

// #6 Suci — court affiliation rendered
await goto('/hr/suci')
const judgeCourtMentions = await countText('/sud:|primarni sud|assigned court/i')
