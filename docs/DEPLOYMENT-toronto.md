...
`~/sudacka-mreza` via `docker-compose.toronto.yml`. This file documents the
...
docker compose -f docker-compose.toronto.yml up -d --build   # cms + ingest
# Frontend: build off-server, then rsync dist/ -> web-dist/ and:
docker compose -f docker-compose.toronto.yml restart web
```
